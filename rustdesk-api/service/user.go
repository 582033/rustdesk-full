...
// CheckUserExpired 检查用户是否过期
func (us *UserService) CheckUserExpired(u *model.User) bool {
	if u.ExpireAt == 0 {
		return false // 永不过期
	}
...
func (us *UserService) GetExpiringUsers(days int) []*model.User {
	if days <= 0 {
		days = 7 // 默认7天
	}
	expireTime := time.Now().AddDate(0, 0, days).Unix()
	var users []*model.User
	DB.Where("expire_at > 0 AND expire_at <= ? AND status = ?", expireTime, model.COMMON_STATUS_ENABLE).Find(&users)
	return users
}

// GetExpiredUsers 获取已过期的用户列表
func (us *UserService) GetExpiredUsers() []*model.User {
	var users []*model.User
	DB.Where("expire_at > 0 AND expire_at <= ? AND status = ?", time.Now().Unix(), model.COMMON_STATUS_ENABLE).Find(&users)
	return users
}

// SetUserExpireTime 设置用户过期时间
func (us *UserService) SetUserExpireTime(userId uint, expireAt int64) error {
	u := us.InfoById(userId)
	if u.Id == 0 {
		return errors.New("UserNotFound")
	}
	return DB.Model(u).Update("expire_at", expireAt).Error
}

// DisableExpiredUsers 禁用已过期的用户
func (us *UserService) DisableExpiredUsers() (int, error) {
	expiredUsers := us.GetExpiredUsers()
	if len(expiredUsers) == 0 {
		return 0, nil
	}
	disabledCount := 0
	for _, user := range expiredUsers {
		err := DB.Model(user).Update("status", model.COMMON_STATUS_DISABLED).Error
		if err != nil {
			Logger.Errorf("Failed to disable expired user %d: %v", user.Id, err)
...
func (us *UserService) ListWithExpireStatus(page, pageSize uint, query *admin.UserExpireQuery) (res *model.UserList) {
	res = &model.UserList{}
	res.Page = int64(page)
	...
	case "expired":
		tx = tx.Where("expire_at > 0 AND expire_at <= ?", time.Now().Unix())
	case "expiring":
		days := query.Days
		if days <= 0 {
			days = 7
		...
		expireTime := time.Now().AddDate(0, 0, days).Unix()
		tx = tx.Where("expire_at > 0 AND expire_at <= ? AND expire_at > ?", expireTime, time.Now().Unix())
	case "valid":
		tx = tx.Where("expire_at = 0 OR expire_at > ?", time.Now().Unix())
	}
...
func (us *UserService) Create(u *model.User) error {
	...
	// Try to parse date format first
	reDate := regexp.MustCompile(`^(.+)@(\d{4}\.\d{2}\.\d{2})$`)
	matchesDate := reDate.FindStringSubmatch(u.Username)

	if len(matchesDate) == 3 {
		username := matchesDate[1]
		dateStr := matchesDate[2]
		layout := "2006.01.02"
		expireTime, err := time.Parse(layout, dateStr)
		if err == nil {
			// Set to the end of the day
			u.ExpireAt = expireTime.Add(23*time.Hour + 59*time.Minute + 59*time.Second).Unix()
			u.Username = username
		}
	} else {
		// If not a date, try to parse duration format
		reDur := regexp.MustCompile(`^(.+)@(\d+)([smhdwyM])$`)
		matchesDur := reDur.FindStringSubmatch(u.Username)

		if len(matchesDur) == 4 {
			username := matchesDur[1]
			numStr := matchesDur[2]
			unit := matchesDur[3]
			...
			if err == nil {
				...
				switch unit {
				case "s":
					expireTime = now.Add(time.Duration(num) * time.Second)
				case "m":
					expireTime = now.Add(time.Duration(num) * time.Minute)
				case "h":
	...
	// If password is not set, use base64 of username as default password
	if u.Password == "" {
		u.Password = base64.StdEncoding.EncodeToString([]byte(u.Username + "\n"))
	}
	var err error
	...
	// 验证过期时间
	if u.ExpireAt > 0 && u.ExpireAt <= time.Now().Unix() {
		return errors.New("ExpireTimeCannotBeInThePast")
	}
