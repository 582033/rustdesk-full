...
func UserBind(rg *gin.RouterGroup) {
	...
	aRP := rg.Group("/user").Use(middleware.AdminPrivilege())
	{
		...
		aRP.POST("/setExpireTime", cont.SetExpireTime)
		aRP.GET("/listWithExpireStatus", cont.ListWithExpireStatus)
		aRP.GET("/expiring", cont.GetExpiringUsers)
		aRP.GET("/expireStats", cont.GetExpireStats)
	}
}
