...
type UserForm struct {
	...
	ExpireAt int64            `json:"expire_at"` // 过期时间戳，0表示永不过期
}
...
// SetExpireTimeForm 设置用户过期时间表单
type SetExpireTimeForm struct {
	UserId   uint  `json:"user_id" validate:"required"`
	ExpireAt int64 `json:"expire_at"` // 过期时间戳，0表示永不过期
}

// UserExpireQuery 用户过期查询表单
type UserExpireQuery struct {
	PageQuery
	ExpireStatus string `form:"expire_status"` // 过期状态：expired(已过期), expiring(即将过期), valid(有效), all(全部)
	Days         int    `form:"days"`          // 即将过期的天数阈值
}
